...
export default function WarunkiScreen() {
  ...
  const [intentionConfirmed, setIntentionConfirmed] = useState<boolean>(false);

  useEffect(() => {
    (async () => {
      const json = await AsyncStorage.getItem(START_KEY);
      if (json) setStartChoice(JSON.parse(json) as StartChoice);
    })();
  }, []);

  const progress = useMemo(() => Object.values(checked).filter(Boolean).length, [checked]);
  const allDone = progress === CONDITIONS.length && intentionConfirmed;
  ...
  return (
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: theme.background }]} style={{ flex: 1 }}>
      ...
      {startChoice && (
        <Card>
          <Text style={{ color: theme.text }}>
            Rozpoczynasz {startChoice.mode === 'zero' ? 'od pierwszej soboty' : `od ${startChoice.nth}. soboty`}.
          </Text>
          {!!startChoice.intention && (
            <Text style={{ color: theme.text, marginTop: 6 }}>Intencja: {startChoice.intention}</Text>
          )}
        </Card>
      )}

      <Card onPress={() => setIntentionConfirmed((v) => !v)}>
        <View style={styles.cardRow}>
          <Text style={styles.cardIcon}>❤️</Text>
          <View style={{ flex: 1 }}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>Intencja wynagradzająca</Text>
            <Text style={[styles.cardSubtitle, { color: theme.text, opacity: 0.8 }]}>
              {startChoice?.intention ?? 'Wynagradzająco Niepokalanemu Sercu Maryi.'}
            </Text>
          </View>
          <View style={[styles.checkbox, { borderColor: intentionConfirmed ? theme.tint : theme.cardBorder, backgroundColor: intentionConfirmed ? theme.tint : 'transparent' }]} />
        </View>
      </Card>
      ...
      <View style={styles.progressRow}>
        ...
        <Text style={{ color: theme.text, marginLeft: 12, fontWeight: '700' }}>Ukończono {progress}/{CONDITIONS.length} + Intencja {intentionConfirmed ? '✓' : '•'}</Text>
      </View>
      ...
      {allDone && (
        <Animated.View style={{ marginTop: 20, padding: 16, borderRadius: 16, borderWidth: 2, borderColor: theme.accentGold, backgroundColor: theme.card }}>
          <Text style={{ textAlign: 'center', fontSize: 18, fontWeight: '800', color: theme.accentGold }}>Dziękujemy! Wszystkie warunki i intencja odznaczone.</Text>
